...
from neon_configuration.util import FileWatcher
from neon_configuration.locations import *


try:
    import mycroft.api as mycroft_api
except ImportError:
    mycroft_api = None
...
class RemoteConf(LocalConf):
    ...
    def reload(self):
        if not mycroft_api:
            LOG.info("Mycroft API module not available")
            self.load_local(self.path)
            return
        try:
            if not mycroft_api.is_paired():
                self.load_local(self.path)
                return

            if mycroft_api.is_backend_disabled():
